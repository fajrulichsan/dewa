...
import com.astra.dewa.model.TokenTable;
import com.astra.dewa.model.impl.TokenImpl;
import com.astra.dewa.model.impl.TokenModelImpl;
import com.astra.dewa.service.persistence.TokenPersistence;
import com.astra.dewa.service.persistence.TokenUtil;
 ...
 * @generated
 */
@Component(service = TokenPersistence.class)
public class TokenPersistenceImpl
	extends BasePersistenceImpl<Token> implements TokenPersistence {

	/*
	 * NOTE FOR DEVELOPERS:
	 *
	 * Never modify or reference this class directly. Always use <code>TokenUtil</code> to access the token persistence. Modify <code>service.xml</code> and rerun ServiceBuilder to regenerate this class.
	 */
	public static final String FINDER_CLASS_NAME_ENTITY =
		TokenImpl.class.getName();
	...
	public TokenPersistenceImpl() {
		...
		setModelImplClass(TokenImpl.class);
		setModelPKClass(int.class);

		setTable(TokenTable.INSTANCE);
	}
	...
	public void cacheResult(List<Token> tokens) {
		if ((_valueObjectFinderCacheListThreshold == 0) ||
			((_valueObjectFinderCacheListThreshold > 0) &&
			 (tokens.size() > _valueObjectFinderCacheListThreshold))) {
	...
	public void clearCache() {
		dummyEntityCache.clearCache(TokenImpl.class);

		dummyFinderCache.clearCache(TokenImpl.class);
	}
	...
	public void clearCache(Token token) {
		dummyEntityCache.removeResult(TokenImpl.class, token);
	}
	...
	public void clearCache(Set<Serializable> primaryKeys) {
		...
		for (Serializable primaryKey : primaryKeys) {
			dummyEntityCache.removeResult(TokenImpl.class, primaryKey);
		}
	}
	...
	protected Token removeImpl(Token token) {
		...
		try {
			...
			if (!session.contains(token)) {
				token = (Token)session.get(
					TokenImpl.class, token.getPrimaryKeyObj());
			}
	...
	public Token updateImpl(Token token) {
		...
		dummyEntityCache.putResult(TokenImpl.class, token, false, true);
	...
	/**
	 * Returns the token with the primary key or throws a <code>com.liferay.portal.kernel.exception.NoSuchModelException</code> if it could not be found.
	 *
	 * @param primaryKey the primary key of the token
	 * @return the token
	 * @throws NoSuchTokenException if a token with the primary key could not be found
	...
	public List<Token> findAll(
		...
		if (useFinderCache) {
			list = (List<Token>)dummyFinderCache.getResult(
		...
		if (list == null) {
			...
			try {
				...
				list = (List<Token>)QueryUtil.list(
	...
	public int countAll() {
		...
		if (count == null) {
			...
			try {
				...
				Query query = session.createQuery(_SQL_COUNT_TOKEN);
	...
	public void activate() {
		...
		_setTokenUtilPersistence(this);
	}

	@Deactivate
	public void deactivate() {
		_setTokenUtilPersistence(null);

		dummyEntityCache.removeCache(TokenImpl.class.getName());
	}

	private void _setTokenUtilPersistence(TokenPersistence tokenPersistence) {
		try {
			Field field = TokenUtil.class.getDeclaredField("_persistence");

			field.setAccessible(true);

			field.set(null, tokenPersistence);
		}
	...
	private static final String _SQL_SELECT_TOKEN =
		"SELECT token FROM Token token";

	private static final String _SQL_COUNT_TOKEN =
		"SELECT COUNT(token) FROM Token token";

	private static final String _ORDER_BY_ENTITY_ALIAS = "token.";

	private static final String _NO_SUCH_ENTITY_WITH_PRIMARY_KEY =
		"No Token exists with the primary key ";

	private static final Log _log = LogFactoryUtil.getLog(
		TokenPersistenceImpl.class);
