package com.astra.dealink.rest.api.application.ReportPlafond;

import com.astra.dewa.model.ReportPlafond;
import com.astra.dealink.rest.api.constants.DewaRestApiKeys;
import com.astra.dealink.rest.api.service.report_plafond.ReportPalfondserviceImpl;
 ...
 * @author psmafifd1401
 */
@ApplicationPath("/report-plafond")
@Component(
      immediate = true,
      service = javax.ws.rs.core.Application.class
)
public class ReportPlafondRestApplication extends Application {
   private final ReportPalfondserviceImpl reportServiceimpl = new ReportPalfondserviceImpl();
   private final Log LOG = LogFactoryUtil.getLog(ReportPlafondRestApplication.class);
   ...
   public Response postReportPalfond(@Context HttpServletRequest request) throws PortalException, ParseException {
...
//                FILE
            ...
            if (!fileName.contains("Report Plafond") || splitterCount != 2) {
...
//                Kode Dealer
            ...
            // Upload
            if (reportServiceimpl.reportPalfondIsExist(fileName, dealerCode)) {
               ...
               try {
                  payload = FolderUtil.uploadFileIntoMenuFolder(file, fileName, group, user, DewaRestApiKeys.REPORT_PLAFOND);
               } catch (Exception e) {
                  LOG.error(e);
                  return Response
                        .status(Response.Status.BAD_REQUEST)
            ...
            try {
               ReportPlafond addReportPlafond = reportServiceimpl.createReportPlafond(dealerId, fileId, fileName, filePath, convertDate);
               LOG.info(addReportPlafond);
            } catch (SystemException e) {
...
//                FolderUtil.moveFileIntoMenuFolder(fileId, fileName, group, user, DewaRestApiKeys.REPORT_PLAFOND);
         } catch (SystemException e) {
