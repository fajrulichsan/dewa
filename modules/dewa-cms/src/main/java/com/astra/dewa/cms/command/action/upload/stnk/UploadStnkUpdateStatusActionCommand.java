...
@Component(
   ...
   property = {
      ...
      "mvc.command.name=upload-stnk-update-status-action"
   },
   service = MVCResourceCommand.class
)
public class UploadStnkUpdateStatusActionCommand extends BaseMVCResourceCommand {

   private final Log log = LogFactoryUtil.getLog(UploadStnkUpdateStatusActionCommand.class);
   private User user;
         ...
         if (approvers.stream().map(MasterApprovalDetail::getUserId).collect(Collectors.toList()).contains(user.getUserId()) ||
            ...
            if (isSequential) {
               ...
               if (status.getId() == ApplicationAssignStatusEnum.APPROVE.getId()) {
                  for (int approverLevel = 0; approverLevel < appAssigns.size(); approverLevel++) {
                     if (appAssigns.get(approverLevel).getProfileId() == user.getUserId()) {
                        // CHECK NEXT ASSIGNEE
                        if (approverLevel < approvers.size() - 1 && header.getApplicationHeaderStatusId() != ApplicationHeaderStatusEnum.WAITING_LEVEL_5.getId()) {
                           ...
                           EmailUploadUtils.sendEmailSubmitToDso(APIConstant.KWITANSI_STNK, header.getTicketNo(), header.getReqDesc(), header.getBusinessBenefit(), header.getNotes(), approvers.get(approverLevel + 1).getUserId());
                        }
                        ...
                        if (MasterApprovalUtils.hasAllAsigneesApproved(header.getApplicationId(), header.getId(), lastSubmitAssignId)) {
                           ...
                           EmailUploadUtils.sendEmailApprovedToHoDealer(APIConstant.KWITANSI_STNK, header.getReqEmail(), header.getReqName(), header.getTicketNo(), header.getReqDesc());
                        }
                     }
                  }
               ...
               if (status.getId() == ApplicationHeaderStatusEnum.APPROVE.getId()) {
                  ...
                  // SEND REVISED EMAIL
                  EmailUploadUtils.sendEmailReviseToHoDealer(APIConstant.KWITANSI_STNK, header.getReqEmail(), header.getReqName(), header.getTicketNo(), header.getReqDesc(), header.getBusinessBenefit(), header.getNotes());
               } else if (status.getId() == ApplicationHeaderStatusEnum.REJECT.getId()) {
                  // SEND REJECTED EMAIL
                  EmailUploadUtils.sendEmailRejectToHoDealer(APIConstant.KWITANSI_STNK, header.getReqEmail(), header.getReqName(), header.getTicketNo(), header.getReqDesc(), header.getBusinessBenefit(), header.getNotes());
               }
