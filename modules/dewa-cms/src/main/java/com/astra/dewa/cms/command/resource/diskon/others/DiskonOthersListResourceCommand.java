...
@Component(
        ...
        property = {
                ...
                "mvc.command.name=" + CmsKeys.CMS_PATH + CmsKeys.REALISASI_DISKON_PATH + "/others/list"
        },
        service = MVCResourceCommand.class
)
public class DiskonOthersListResourceCommand extends BaseMVCResourceCommand {
    ...
    protected void doServeResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse) throws Exception {
        ...
        try {
            ...
            switch (userRoleGroup) {
                case HO_DEALER:
                case DEALER:
                    throw new PortalException("The selected user has no authority to access this resource.");
            }
            // filter query
            if (dealerId > 0) {
            ...
            for (DiskonOther diskonOther : r) {
                try {
                    ...
                    if (Validator.isNotNull(diskonOther.getModifiedDate())) {
                        uploadDateAsString = DateUtil.dateToString(diskonOther.getModifiedDate(), CmsKeys.DATE_FORMAT_SLASH_24_H);
                    }
