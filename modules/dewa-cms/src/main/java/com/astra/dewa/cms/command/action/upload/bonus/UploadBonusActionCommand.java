...
@Component(
      ...
      property = {
            ...
            "mvc.command.name=upload-bonus-action"
      },
      service = MVCResourceCommand.class
)
public class UploadBonusActionCommand extends BaseMVCResourceCommand {

   private final Log log = LogFactoryUtil.getLog(UploadBonusActionCommand.class);
...
//               EmailUploadBonusUtil.sendEmailSubmitToDso(APIConstant.KWITANSI_BONUS, header.getTicketNo(), header.getReqDesc(), header.getBusinessBenefit(), header.getNotes());
//               EmailUploadBonusUtil.sendEmailSubmitToHoDealer(APIConstant.KWITANSI_BONUS, user.getEmailAddress(), user.getFullName(), header.getTicketNo(), header.getReqDesc(), header.getBusinessBenefit(), header.getNotes());
...
//               EmailUploadBonusUtil.sendEmailSubmitToHoDealer(APIConstant.KWITANSI_BONUS, user.getEmailAddress(), user.getFullName(), header.getTicketNo(), header.getReqDesc(), header.getBusinessBenefit(), header.getNotes());
   ...
   private JSONObject update(ApplicationHeader header) {
      try {
         ...
         createApplicationHeaderJournal(header, user.getScreenName());
         if (header.getApplicationHeaderStatusId() != ApplicationHeaderStatusEnum.DRAFT.ordinal() + 1) {
