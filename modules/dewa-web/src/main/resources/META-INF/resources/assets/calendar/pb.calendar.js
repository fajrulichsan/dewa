jQuery(function($){
	...
	window.PB_CALENDAR_DEFAULTS = {
		...
		'min_yyyymm' : null,
		'max_yyyymm' : null,
		'next_month_button' : '<i class="fa fa-chevron-right"></i>',
		'prev_month_button' : '<i class="fa fa-chevron-left"></i>',
	...
	_pb_calendar.prototype.update_view = (function(){
		...
		for(var day_index_= 0; day_index_ <= day_count_; ++day_index_){
			...
			result_html_ += '<div class="schedule-dot-list '+target_moment_.format("YYYYMMDD")+'"></div>';
		...
		this._schedule_list(function(schedule_list_){
			...
			$.each(schedule_list_, function(yyyymmdd_, schedule_data_){
				...
				//custom
				var $html = '<div class="dropdown" id="'+yyyymmdd_+'"><div class="dropdown-menu">'+
					        '<div class="">';
				$.each(schedule_data_, function(){
					$html += '<div class="panel-body bdb-gray bdl-blue">'+
					                '<strong class="text-bold">'+this['data']+'</strong>'+
					                '<p class="text-white"><small>test</small></p>'+
					            '</div>';
				});
				$html += '</div></div></div>';
				schedule_dot_list_el_.append($html);
				/**/
				$.each(schedule_data_, function(){
