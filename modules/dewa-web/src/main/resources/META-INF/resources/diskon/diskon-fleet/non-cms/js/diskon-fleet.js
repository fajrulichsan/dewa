...
var DISKON_FLEET_NON_CMS_TABLE_NAME = "#diskon_fleet_non_cms_table";
var diskonFleetDataTable = $(DISKON_FLEET_NON_CMS_TABLE_NAME).DataTable({
   searching: false,
   paging: true,
   info: false,
   responsive: true,
   ...
   ajax: {
      ...
      dataSrc: function (json) {
         ...
         tableChecker($(window).width(), DISKON_FLEET_NON_CMS_TABLE_NAME, length);
         */
         return json.Data;
      }
   },
   columns: [
      ...
      {data: "kuartalName", "width": "20%", className: "text-center"},
      {
         data: "uploadDate",
         "width": "30%",
...
function renderActionButton(dataId, row) {
   var downloadURL = new URL(DOWNLOAD_DISKON_FLEET_URL);
...
$(window).on("resize", function () {
   ...
   if (width < 768) {
      diskonFleetDataTable.page.len(10).draw();
   }
   // tableChecker(width, DISKON_FLEET_NON_CMS_TABLE_NAME, length);
});