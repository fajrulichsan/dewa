...
<portlet:resourceURL id="upload-bonus-monitoring-request-action" var="uploadBonusActionURL"/>
<portlet:resourceURL id="upload-file-upload-bonus-monitoring" var="uploadFileUploadBonusCommandURL"/>
<portlet:resourceURL id="delete-file-upload-bonus-monitoring" var="deleteFileUploadBonusCommandURL"/>

<portlet:resourceURL id="/non-cms/bonus-application-category" var="requestUploadBonusURL"/>
<portlet:resourceURL id="upload-bonus-monitoring-application-header-status" var="statusUploadBonusURL"/>
   ...
   <div class="row">
      <div class="col-md-6">
         <div class="form-group" style="margin-left: 1px;margin-right: 1px;">
            ...
            <select class="form-control" name="requestBonusId" id="requestBonusId" style="width: 100%;"></select>
...
<script>
   ...
   $(document).ready(function () {
      $('.title-breadcrumb').text('Home > Claim Dealer > Kuitansi Bonus > Ubah Request');

      var threeYearsAgo = new Date().getFullYear() - 3;
      if (data.reqYearId < threeYearsAgo) {
         Swal.fire({
            icon: 'error',
            title: 'Dokumen tidak bisa di edit',
            text: 'Request tahun document sudah melewati batas tiga tahun kebelakang',
            confirmButtonText: 'Back',
            allowOutsideClick: false
         }).then((result) => {
            if (result.isConfirmed) {
               window.location.href = '/group/dealink/kuitansi-bonus/monitoring-request';
            }
         });
      } else {
         getRequestUploads();
   ...
   function formEdit() {
      ...
      if (data.statusBonusId === 4 || data.statusBonusId === 5) {
         $('#btnSubmit').show();
         $('#btnDraft').show();
      } else {
         $('#btnSubmit').hide();
   ...
   function deleteFile() {
      Swal.fire({
         ...
         if (result.isConfirmed) {
            ...
            formData.set("crudType", 'deleteFile');
            formData.set("p_auth", Liferay.authToken);
            $.ajax({
               url: '${uploadBonusActionURL}',
               type: 'POST',
   ...
   function getReqYears() {
      ...
      reqYearConst.val(data.reqYearId);
