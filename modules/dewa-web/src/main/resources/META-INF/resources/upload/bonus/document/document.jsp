...
<portlet:resourceURL id="upload-bonus-document-list-nonCMS" var="uploadBonusDocumentListURL"/>

<portlet:resourceURL id="upload-bonus-document-dealer" var="dealerUploadBonusURL"/>
<portlet:resourceURL id="upload-bonus-document-application-category" var="requestUploadBonusURL"/>
<%--
<portlet:resourceURL id="upload-bonus-document-application-assign-status" var="statusUploadBonusURL"/>
--%>
<portlet:resourceURL id="/upload-request/status" var="statusUploadBonusURL"/>
<portlet:resourceURL id="upload-bonus-document-tahun" var="tahunUploadBonusURL"/>
...
<style>
    ...
    #uploadBonusDocument img {
        padding: 5px 0;
    }

    #uploadBonusDocument h4 {
        font-weight: 300;
        padding: 0 9px;
        margin: 0 0;
    }

    #uploadBonusDocument p {
        font-weight: 300;
        padding: 0 9px;
        margin: 0 0;
    }

    #uploadBonusDocument a {
        font-weight: 700;
        /*padding: 0 16px;*/
    }
...
<section id="uploadBonusDocument">
   <div class="row">
      <h5 class="title-breadcrumb"></h5>
   </div>

   <div class="row">
      <h5 class="title-format">Hasil Upload Dokumen</h5>
   </div>
   ...
   <div class="row" style="margin-bottom: 15px;">
      ...
      <div class="col-xs-12 col-sm-12 col-md-2 ticketList" style="margin-bottom: 5px;">
   ...
   <div class="row" style="margin: 0px;" id="bonusList"></div>
...
<script>
   ...
   $(document).ready(function () {
      $('.title-breadcrumb').text('Home > Claim Dealer > Kuitansi Bonus > Hasil Upload Dokumen');

      tuiInstance.on('beforeMove', (event) => {
         currentPage = event.page;
         itemRow = tuiInstance._options.itemsPerPage;
         getUploadBonus(currentPage, itemRow, false);
      });
      refresh();
   });
      ...
      $.ajax({
         ...
         success: function (response) {
            ...
            for (var i = 0; i < dataSet.length; i++) {
               ...
               downloadURL.searchParams.set('id', dataSet[i].id);
               downloadURL.searchParams.set('p_auth', Liferay.authToken);
               list += '<div class="col-xs-12 col-sm-4 col-md-3 col-lg-3 card-document" >';
               list += '<div class="card">';
               ...
               list += '<a href="' + downloadURL.toString() + '" target="_blank">';
               // list += '<a href="' + dataSet[i].fileUrl + '?download=true" target="_blank">';
               list += '<img src="<%=request.getContextPath()%>/assets/img/file_download.svg" style="width:35px;height:35px;margin:0;">';
               list += '</a>';
               list += '</div>';
               list += '</div>';
               // }
