...
$(document).ready(function () {
   tuiInstance.on('beforeMove', (event) => {
      ...
      getUploadStnkCards(currentPage, itemRow);
   });
   refresh();
});
...
function getUploadBonus() {
   createLoading();
   uploadStnkDataTable = $('#upload_stnk_table').DataTable({
      destroy: true,
      processing: true,
      serverSide: true,
      paging: true,
...
function actionButton(dataId, applicationHeaderStatusId) {
   var editURL = new URL(UPLOAD_STNK_EDIT_URL);
   ...
   var response = "";
   if ((applicationHeaderStatusId === 0 || applicationHeaderStatusId === 4 || applicationHeaderStatusId === 5)) {
...
function getUploadStnkCards(currentPage, itemRow) {
   createLoading();
   $.ajax({
      url: UPLOAD_STNK_LIST_CARD_URL,
      type: "GET",
      data: {
         column: column,
         keyword: keyword,
...
function uploadStnkCards(data) {
   $('#upload_stnk_cards').html('');
   var upload_stnk_cards = [];
   for (var i = 0; i < data.length; i++) {
      var dfrv =
         '<div class="upload_stnk_card">' +
            '<div class="row">' +
               '<div class="col-xs-9">' +
            ...
            '</div>' +
         '</div>';
      upload_stnk_cards.push(dfrv);
   }
   $('#upload_stnk_cards').append($(upload_stnk_cards.join('')));
}
   ...
   if (notes === null || notes === undefined || notes.length < 9) {
      ...
      Swal.fire({
         ...
         if (result.isConfirmed) {
            ...
            $.ajax({
               url: UPLOAD_STNK_UPDATE_STATUS_ACTION_URL,
               type: "POST",
               data: formData,
               processData: false,
               contentType: false,
