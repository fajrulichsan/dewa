package com.astra.dewa.web.command.render.diskon.fleet.noncms.util;
...
@Component(
        ...
        property = {
                ...
                "mvc.command.name=/kuartal-diskon-fleet-non-cms"
        },
        service = MVCResourceCommand.class
)
public class KuartalDiskonFleetNonCMSRenderCommand extends BaseMVCResourceCommand {

    private final Log log = LogFactoryUtil.getLog(KuartalDiskonFleetNonCMSRenderCommand.class);
    private String role;
    ...
    private List<String> getEntryKuartalList() {
        ...
        try {
            ...
            diskonFleets.forEach(data -> {
                if (!kuartalList.contains(data.getKuartal())) {
                    kuartalList.add(data.getKuartal());
                }
            });
        } catch (Exception e) {
            log.error(e);
