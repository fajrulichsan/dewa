...
@Component(
   property = {
      ...
      "mvc.command.name=/upload-file-diskon-test-car"
   },
   service = MVCResourceCommand.class
)

public class UploadFileDiskonTestCarCommand extends BaseMVCResourceCommand {
   private final Log LOGGER = LogFactoryUtil.getLog(UploadFileDiskonTestCarCommand.class);
   ...
   protected void doServeResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse) throws Exception {
      ...
      String tipeKendaraanName = ParamUtil.getString(uploadPortletRequest, "tipeKendaraanName");

      File file = uploadPortletRequest.getFile("file-upload");
      HashMap<String, String> payload = new HashMap<>();

      fileName = DewaWebKeys.DISKON_TEST_CAR_MENU + " " + tipeKendaraanName + "-" + kuartalName + "-" + tahun + "-" + dealerCode + "." + FileUtil.getExtensionByStringHandling(fileName).get();

      try {
//         payload = FileUtil.uploadFileIntoMenuFolder(file, fileName, themeDisplay, resourceRequest, DewaWebKeys.TEMP_FOLDER);
         payload = FileUtil.uploadFileIntoMenuFolder(file, fileName, themeDisplay, resourceRequest, DewaWebKeys.DISKON_TEST_CAR_MENU);
      } catch (Exception e) {
         acknowledge = 0;
         status = "error";
