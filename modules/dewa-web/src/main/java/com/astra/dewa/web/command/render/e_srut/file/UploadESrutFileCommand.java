package com.astra.dewa.web.command.render.e_srut.file;
...
@Component(
      property = {
            ...
            "mvc.command.name=/upload-e-srut-file"
      },
      service = MVCResourceCommand.class
)
public class UploadESrutFileCommand extends BaseMVCResourceCommand {
   private final Log LOG = LogFactoryUtil.getLog(UploadESrutFileCommand.class);
   ...
   protected void doServeResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse) throws Exception {
      ...
      try {
         LOG.info("==== Uploading E-SRUT File ====");
         // CSRF AUTHENTICATION
         ...
         if (splitterCount != 1 || !fileName.toLowerCase().startsWith(DewaWebKeys.SRUT.toLowerCase())) {
            acknowledge = 0;
            status = "error";
            message = "Maaf, format nama file yang anda masukkan tidak valid. Gunakan format yang sesuai : SRUT-NOKA.pdf";
         } else {
            try {
               ...
               payload = FileUtil.uploadFileIntoMenuFolder(file, fileName, themeDisplay, resourceRequest, DewaWebKeys.E_SRUT_MENU);
            }
