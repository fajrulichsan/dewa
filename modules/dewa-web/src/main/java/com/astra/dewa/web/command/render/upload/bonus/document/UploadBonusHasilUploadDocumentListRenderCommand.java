package com.astra.dewa.web.command.render.upload.bonus.document;
...
@Component(
      ...
      property = {
            ...
            "mvc.command.name=upload-bonus-document-list-nonCMS"
      },
      service = MVCResourceCommand.class
)
public class UploadBonusHasilUploadDocumentListRenderCommand extends BaseMVCResourceCommand {
   private final Log LOG = LogFactoryUtil.getLog(UploadBonusHasilUploadDocumentListRenderCommand.class);
   ...
   protected void doServeResource(ResourceRequest request, ResourceResponse response) throws Exception {
      ...
      try {
         ...
         // COUNTER
         DynamicQuery dynamicQuery = ApplicationHeaderLocalServiceUtil.dynamicQuery();
         dynamicQuery.add(RestrictionsFactoryUtil.eq("ApplicationId", ApplicationEnum.UPLOAD_BONUS.ordinal() + 1));
         ...
         if (!column.equals("ALL") && !keyword.equals("ALL")) {
            switch (column) {
               case "REQUEST_CATEGORY":
                  ...
                  dynamicQuery.add(RestrictionsFactoryUtil.eq("ApplicationCategoryId", Integer.parseInt(keyword)));
                  break;
               case "DEALER_NAME":
                  query.add(RestrictionsFactoryUtil.eq("DealerId", Integer.parseInt(keyword)));
                  dynamicQuery.add(RestrictionsFactoryUtil.eq("DealerId", Integer.parseInt(keyword)));
                  break;
               case "TICKET_NUMBER":
                  query.add(RestrictionsFactoryUtil.like("TicketNo", "%" + keyword + "%"));
                  dynamicQuery.add(RestrictionsFactoryUtil.like("TicketNo", "%" + keyword + "%"));
                  break;
               case "TICKET_DATE":
                  ...
                  dynamicQuery.add(RestrictionsFactoryUtil.eq("ReqYear", date.getYear()));
                  dynamicQuery.add(RestrictionsFactoryUtil.eq("ReqMonth", month));
                  break;
               case "STATUS":
                  query.add(RestrictionsFactoryUtil.eq("ApplicationHeaderStatusId", Integer.parseInt(keyword)));
                  dynamicQuery.add(RestrictionsFactoryUtil.eq("ApplicationHeaderStatusId", Integer.parseInt(keyword)));
                  break;
               case "TAHUN":
                  query.add(RestrictionsFactoryUtil.eq("ReqYear", Integer.parseInt(keyword)));
                  dynamicQuery.add(RestrictionsFactoryUtil.eq("ReqYear", Integer.parseInt(keyword)));
                  break;
               default:
            }
         }
         ...
         if (!isSuperUser) {
            ...
            dynamicQuery.add(RestrictionsFactoryUtil.eq("ReqScreenName", user.getScreenName()));
         }
         ...
         long recordsTotal = ApplicationHeaderLocalServiceUtil.dynamicQueryCount(dynamicQuery);
         ...
         for (ApplicationHeader bonus : bonuses) {
            ...
            // Request Information
//            if(bonus.getApplicationCategoryId() > 0) {
...
//               dto.put("requestBonusId", requestBonus.getId());
//               dto.put("requestBonusName", requestBonus.getName());
//            } else {
...
//            }
            ...
            String thumbnailURL = themeDisplay.getPortalURL() + themeDisplay.getPathContext() + "/documents/" +
                  themeDisplay.getScopeGroupId() + "/" + fileEntry.getFolderId() + "/" + fileEntry.getFileName();
            dto.put("fileUrl", thumbnailURL);
            ...
            if (bonus.getApplicationHeaderStatusId() > 0) {
               ...
               dto.put("statusBonusId", statusBonus.getId());
               dto.put("statusBonusName", statusBonus.getName());
            } else {
