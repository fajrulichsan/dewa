package com.astra.dewa.web.command.render.surat_penalty_stock.file;
...
@Component(
   property = {
      ...
      "mvc.command.name=/upload-file-surat-penalty-stock"
   },
   service = MVCResourceCommand.class
)

public class UploadFileSuratPenaltyStockCommand extends BaseMVCResourceCommand {
   private final Log LOG = LogFactoryUtil.getLog(UploadFileSuratPenaltyStockCommand.class);
   ...
   protected void doServeResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse) throws Exception {
      ...
      try {
         ...
         fileName = fileName.substring(0, fileName.lastIndexOf(".")) + "_SPS_" + sdf.format(new Date()) + "." + FileUtil.getExtensionByStringHandling(fileName).get();

         if (fileId == 0) {
            payload = FileUtil.uploadFileIntoMenuFolder(file, fileName, themeDisplay, resourceRequest, DewaWebKeys.SURAT_PENALTY_STOCK_MENU);
         } else {
            try {
