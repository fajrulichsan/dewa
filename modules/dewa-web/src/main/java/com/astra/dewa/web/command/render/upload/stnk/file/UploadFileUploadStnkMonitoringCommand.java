package com.astra.dewa.web.command.render.upload.stnk.file;
...
@Component(
   property = {
      ...
      "mvc.command.name=upload-file-upload-stnk-monitoring"
   },
   service = MVCResourceCommand.class
)

public class UploadFileUploadStnkMonitoringCommand extends BaseMVCResourceCommand {
   private final Log LOG = LogFactoryUtil.getLog(UploadFileUploadStnkMonitoringCommand.class);
   ...
   protected void doServeResource(ResourceRequest request, ResourceResponse response) throws Exception {
      ...
      try {
         ...
         if (fileId == 0) {
            payload = FileUtil.uploadFileIntoMenuFolder(file, fileName, themeDisplay, request, DewaWebKeys.UPLOAD_COPY_STNK);
         } else {
            try {
