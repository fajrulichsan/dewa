...
@Component(
      ...
      property = {
            ...
            "mvc.command.name=upload-bonus-request-action"
      },
      service = MVCResourceCommand.class
)
public class UploadBonusRequestActionCommand extends BaseMVCResourceCommand {
   private final Log LOG = LogFactoryUtil.getLog(UploadBonusRequestActionCommand.class);
   private User user;
   ...
   protected void doServeResource(ResourceRequest request, ResourceResponse response) throws Exception {
      ...
      try {
         ...
         if (isRequestContainsXSS) {
            ...
            int reqYearId = ParamUtil.getInteger(portletRequest, "reqYearId");
            ...
            if (Validator.isNull(nominalPengajuan)) {
               throw new SystemException("Nominal belum diisi");
            }
            if (!InputValidationUtils.isNumberOnly(nominalPengajuan)) {
               throw new SystemException("Mohon gunakan angka numeric pada Nominal Pengajuan");
            }
            if (Integer.parseInt(nominalPengajuan) < 0) {
               throw new SystemException("Mohon input nominal yang sesuai");
            }
...
//            String attachmentPath = ParamUtil.getString(portletRequest, "attachmentPath");
            ...
            boolean isSequential = MasterApprovalUtils.isSequential(ApplicationEnum.UPLOAD_BONUS.ordinal() + 1);
            ...
            if (crudType.equalsIgnoreCase(CREATE)) {
               ...
               if (ApplicationHeaderStatusEnum.isWaiting(status.getId())) {
                  ...
                  List<MasterApprovalDetail> approvers = MasterApprovalUtils.getApprovers(MasterApprovalUtils.getMasterApprovalId(ApplicationEnum.UPLOAD_BONUS.ordinal() + 1));
                  ...
                  EmailUploadUtils.sendEmailSubmitToHoDealer(APIConstant.KWITANSI_BONUS, user.getEmailAddress(), user.getFullName(), header.getTicketNo(), header.getReqDesc(), header.getBusinessBenefit(), header.getNotes());
               ...
               jsonObject = create(header, isSequential);
            }
         }
      } catch (Exception e) {
   ...
   private JSONObject create(ApplicationHeader header, boolean isSequential) {
      try {
         Application application = ApplicationLocalServiceUtil.getApplication(ApplicationEnum.UPLOAD_BONUS.ordinal() + 1);
