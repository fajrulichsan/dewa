...
@Component(
      ...
      property = {
            ...
            "mvc.command.name=/tipe-kendaraan-diskon-test-car-non-cms"
      },
      service = MVCResourceCommand.class
)
public class TipeKendaraanDiskonTestCarNonCMSRenderCommand extends BaseMVCResourceCommand {

   private final Log log = LogFactoryUtil.getLog(TipeKendaraanDiskonTestCarNonCMSRenderCommand.class);
   private String role;
   ...
   private JSONArray vehicleList() {
      List<Integer> entryVehicleList = getEntryVehicleList();
      JSONArray jsonData = JSONFactoryUtil.createJSONArray();
      try {
         for (Integer id : entryVehicleList) {
            JSONObject jsonObject = JSONFactoryUtil.createJSONObject();
            String tipeKendaraanName = TipeKendaraanLocalServiceUtil.getTipeKendaraan(id).getName();
            jsonObject.put("id", id);
            jsonObject.put("text", tipeKendaraanName);
   ...
   private List<Integer> getEntryVehicleList() {
      ...
      try {
         ...
         diskonTestCars.forEach(data -> {
            if (!vehicleList.contains(data.getTipeKendaraanId())) {
               vehicleList.add(data.getTipeKendaraanId());
            }
         });
      } catch (Exception e) {
         log.error(e);
