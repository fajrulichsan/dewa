...
@Component(
      property = {
            ...
            "mvc.command.name=/upload-sales-program"
      },
      service = MVCResourceCommand.class
)

public class UploadFileSalesProgramCommand extends BaseMVCResourceCommand {
   private final Log LOG = LogFactoryUtil.getLog(UploadFileSalesProgramCommand.class);
   ...
   protected void doServeResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse) throws Exception {
      ...
      try {
         ...
         if (fileId == 0) {
            payload = FileUtil.uploadFileIntoMenuFolder(file, fileName, themeDisplay, resourceRequest, DewaWebKeys.SALES_PROGRAM_MENU);
         } else {
            try {
